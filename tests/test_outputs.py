...
1. Achieved 3x speedup in PyTorch DDP training
2. Created an optimization report
3. Properly configured NCCL to use RDMA (not TCP fallback)
"""
...
def test_baseline_timing_exists():
    ...
    assert baseline_file.exists(), (
        "baseline_timing.txt not found. Did you run the baseline benchmark?"
    )
    ...
    assert baseline_time > 0.1, (
        f"Baseline time seems too fast: {baseline_time}s (expected >0.1s)"
    )
...
def test_optimized_timing_exists():
    ...
    assert optimized_file.exists(), (
        "optimized_timing.txt not found. Did you run the optimized PyTorch DDP test?"
    )
...
def test_optimization_report_exists():
    ...
    assert report_file.exists(), (
        "optimization_report.md not found. Please create this file documenting your optimizations."
    )
...
def test_optimization_report_content():
    ...
    required_topics = {
        ...
        "configuration": ["configuration", "environment", "variable"],
    }
    ...
    if missing_topics:
        print(
            f"  Warning: Report could discuss more about: {', '.join(missing_topics)}"
        )
    else:
...
def test_nccl_environment_configured():
    ...
    if found_vars:
        ...
        print("  Info: NCCL configuration could be more explicit")
